...
def calculate_order_binom_fast():
    ...
    # Start from run 10 for each test
    index_run = np.full(config.TESTS, 1, dtype=int)

    # Initialize priority queue with the next available likelihood for each test
    ...
    for _ in range(total_iterations):
        ...
        if index_run[test_id] < config.RUNS:
            ...
            next_likelihood = np.inf  # Max to avoid being picked again
